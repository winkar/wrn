...
def query_from_db(text):
    ...
    try:
        ...
        for line in result:
            taskId = str(line[0]).replace(text, "\033[94m%s\033[0m" % text)
            task = line[1].replace(text, "\033[94m%s\033[0m" % text)
            cmd = line[2]
            output=line[3] + '\n'
            tag = ""
            if line[4].strip():
                tag = "tag: "+ line[4].replace(text, "\033[94m%s\033[0m" % text)
...
def main():
    ...
    output = ""
    cmd = options["cmd"]
    print("Cmd executed: {} 2>&1".format(cmd))
